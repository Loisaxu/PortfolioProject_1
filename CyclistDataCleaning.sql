...
-- First, take a peak -- 
SELECT 
	start_time, date_format(STR_TO_DATE(start_time, '%Y/%m/%d %H:%i'), '%Y/%m/%d %H:%i')AS start_date_time,
    end_time, date_format(STR_TO_DATE(end_time, '%Y/%m/%d %H:%i'), '%Y/%m/%d %H:%i') AS end_date_time
FROM tripsInfo
...
UPDATE tripsInfo
SET start_time = date_format(STR_TO_DATE(start_time, '%Y/%m/%d %H:%i'), '%Y/%m/%d %H:%i');

UPDATE tripsInfo
SET end_time = date_format(STR_TO_DATE(end_time, '%Y/%m/%d %H:%i'), '%Y/%m/%d %H:%i');
...
UPDATE tripsInfo
SET start_date = DATE_FORMAT(start_time, '%Y/%m/%d');

UPDATE tripsInfo
SET start_time_slot = DATE_FORMAT(start_time, '%H:%i');

-- Voila, let's check the result -- 
SELECT *
FROM tripsInfo
WHERE TIME(start_time_slot) > '20:00:00';
...
UPDATE tripsInfo
SET is_subscriber = CASE WHEN usertype = 'Subscriber' THEN 'Yes' ELSE 'No' END;

--------------------------
-- Check for duplicates --
--------------------------
SELECT trip_id, COUNT(trip_id) AS duplex_trips
FROM tripsInfo
GROUP BY trip_id 
